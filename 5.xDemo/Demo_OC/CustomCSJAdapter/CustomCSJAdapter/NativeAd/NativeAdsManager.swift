...
class NativeAdsManager: NativeAdProtocol {
    
    weak var adapter: AWMCustomNativeAdapter?
    weak var bridge: AWMCustomNativeAdapterBridge?
    var nativeAdDataArray: [BUNativeAd]?
    var loadListener: NativeAdLoaderListener?
    var nativeAdsManager: BUNativeAdsManager?
    ...
    private func setup() {
        self.loadListener = NativeAdLoaderListener(bridge: bridge, adapter: adapter)
    }
    ...
    func loadAd(placementId: String, adSize: CGSize, parameter: WindMillSDK.AWMParameter) {
        ...
        self.nativeAdsManager = BUNativeAdsManager(slot: adSlot)
        nativeAdsManager?.delegate = self.loadListener
        let count = parameter.extra[WindMillConstant.LoadAdCount] as? Int ?? 1
        nativeAdsManager?.loadAdData(withCount: count)
    }
